...
Pipeline IA → XML nativo de Microsoft Project → `.mpp` para resolver la **Actividad 2 — Laboratorio: planificación, ejecución y gestión del cambio** (UNIR, Especialización en Inteligencia Artificial, *Control de Proyectos TI*).

## Estructura

```
.
├── docs/                      # Enunciado, guía oficial y PDF de investigación
│   ├── Actividad2_Laboratorio_MS_Project_BORRADOR.docx
│   ├── digete05_t7_lab.docx
│   └── Quiero que investigues procesos de creación de dia.pdf
└── solution/
...
- **Ruta A (principal) — XML nativo.** `solution/xml/proyecto-vr.xml` transporta EDT + Calendario Colombia + recursos + asignaciones. Se abre en MS Project Pro 2021 y se guarda como `.mpp`.
- **Ruta C (respaldo) — CSV/Excel.** Si el XML fallara, los CSV de `solution/data/` sirven como fuente para el asistente oficial *Importar datos de Excel a Project* (ref: https://support.microsoft.com/es-es/topic/importar-datos-de-excel-a-project-cb3fb91a-ad05-4506-b0af-8aa8b2247119).
...
El XML ya incluye en `solution/data/asignaciones.csv` la asignación base de `Modelador y texturizador 3D` a 1.2.2 Texturizado. Para simular el cambio:
...
Además, reemplazar en el texto del informe **Calendario Madrid → Calendario Colombia** y la lista de festivos por la de `solution/data/festivos.csv`, ya que se decidió usar festivos colombianos.

## Notas sobre asignaciones

`solution/data/asignaciones.csv` contiene asignaciones razonables por el nombre de cada tarea; el BORRADOR menciona una tabla de asignaciones del enunciado que no está incluida en el documento. Si el enunciado original difiere, basta con editar el CSV y volver a ejecutar `build_xml.py`.
